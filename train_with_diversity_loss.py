...
from discriminator import Discriminator,TensorBuffer
...
class Trainer:
    def train_epoch(self, disc_H, disc_Z, gen_Z, gen_H, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler):
        ...
        image_buffer_zebra = TensorBuffer()
        image_buffer_horse = TensorBuffer()
        ...
        for idx, (zebra, horse) in enumerate(loop):
            ...
            with torch.cuda.amp.autocast():
                ...
                image_buffer_zebra.push(fake_zebra)
                ...
                image_buffer_horse.push(fake_horse)
            ...
            buffer_zebra=image_buffer_zebra.get_all()
            buffer_horse=image_buffer_horse.get_all()

            mean_buffer_horse = buffer_horse.mean(dim=0).to(fake_horse.dtype).to(fake_horse.device)
            mean_buffer_zebra = buffer_zebra.mean(dim=0).to(fake_zebra.dtype).to(fake_zebra.device)

            diversity_loss_horse = mse(fake_horse, mean_buffer_horse)
            diversity_loss_zebra = mse(fake_zebra, mean_buffer_zebra)
            diversity_loss = diversity_loss_horse + diversity_loss_zebra

            G_loss = (config.LAMBDA_ADVERSERIAL * (loss_G_Z + loss_G_H) + config.LAMBDA_CYCLE * (
